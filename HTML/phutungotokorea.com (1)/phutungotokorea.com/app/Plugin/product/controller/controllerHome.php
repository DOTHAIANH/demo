<?php
	...
	function search($input)
	{
		...
		// /tag/key/category/price/sort

		// sửa link cũ
		if(!empty($_GET['key'])){
			$modelProduct->redirect('/tag/'.createSlugMantan($_GET['key']) );
		}

		if(empty($_GET['key']) && !empty($input['request']->params['pass'][1])){
			$_GET['key'] = $input['request']->params['pass'][1];
		}else{
			$_GET['key'] = '';
		}

		if(empty($_GET['category']) && !empty($input['request']->params['pass'][2])){
			$_GET['category'] = $input['request']->params['pass'][2];
		}

		if(empty($_GET['price']) && !empty($input['request']->params['pass'][3])){
			$_GET['price'] = $input['request']->params['pass'][3];
		}

		if(empty($_GET['sort']) && !empty($input['request']->params['pass'][4])){
			$_GET['sort'] = $input['request']->params['pass'][4];
		}
		...
		if(isset($seoProduct['Option']['value']['search']['title']) && $seoProduct['Option']['value']['search']['title']!='')
		{
			...
			$metaTitleMantan= str_replace('%description%', $metaDescriptionMantanDefault, $metaTitleMantan);
			if(isset($_GET['key'])){
				$metaTitleMantan= str_replace('%keySearch%', str_replace('-', ' ', $_GET['key']), $metaTitleMantan);
			}
			$metaTitleMantan= str_replace('%page%', $page, $metaTitleMantan);
		...
		if(isset($seoProduct['Option']['value']['search']['keyword']) && $seoProduct['Option']['value']['search']['keyword']!='')
		{
			...
			$metaKeywordsMantan= str_replace('%description%', $metaDescriptionMantanDefault, $metaKeywordsMantan);
			if(isset($_GET['key'])){
				$metaKeywordsMantan= str_replace('%keySearch%', str_replace('-', ' ', $_GET['key']), $metaKeywordsMantan);
			}
			$metaKeywordsMantan= str_replace('%page%', $page, $metaKeywordsMantan);
		...
		if(isset($seoProduct['Option']['value']['search']['description']) && $seoProduct['Option']['value']['search']['description']!='')
		{
			...
			$metaDescriptionMantan= str_replace('%description%', $metaDescriptionMantanDefault, $metaDescriptionMantan);
			if(isset($_GET['key'])){
				$metaDescriptionMantan= str_replace('%keySearch%', str_replace('-', ' ', $_GET['key']), $metaDescriptionMantan);
			}
			$metaDescriptionMantan= str_replace('%page%', $page, $metaDescriptionMantan);
		...
		if(isset($_GET['key']) && $_GET['key']!='')
		{
			...
			$conditions['$or'][2]['code']= array('$regex' => $_GET['key']);
			$conditions['$or'][3]['alias']= array('$regex' => $_GET['key']);
		}
		// tim theo gia
	...
	function productDetail($input)
	{
		...
		else
		{
			...
			if(isset($seoProduct['Option']['value']['detail']['title']) && $seoProduct['Option']['value']['detail']['title']!='' && $data['Product']['nameSeo']=='')
			{
				...
				$metaTitleMantan= str_replace('%productCode%', $data['Product']['code'], $metaTitleMantan);
			}
			...
			if(isset($seoProduct['Option']['value']['detail']['keyword']) && $seoProduct['Option']['value']['detail']['keyword']!='')
			{
				...
				$metaKeywordsMantan= str_replace('%productCode%', $data['Product']['code'], $metaKeywordsMantan);
			}
			...
			if(isset($seoProduct['Option']['value']['detail']['description']) && $seoProduct['Option']['value']['detail']['description']!='')
			{
				...
				$metaDescriptionMantan= str_replace('%productCode%', $data['Product']['code'], $metaDescriptionMantan);
			}
		}
		...
		//$metaTitleMantan .= ' '.$data['Product']['code'];
	...
	function saveUser($input)
	{
		...
		if(isset($dataSend['fullname']) && isset($dataSend['username']) && isset($dataSend['email']) && isset($dataSend['phone']) && isset($dataSend['address']) && isset($dataSend['password']) && isset($dataSend['passwordAgain']) && !empty($dataSend['captcha'])){
			if($dataSend['captcha']==@$_SESSION['capchaCodeRegister']){
				$save= array();
			...
			else $modelUser->redirect(getLinkRegister().'?status=-4');
		}else{
			$modelUser->redirect($urlHomes);
		}
	}
	...
	{
		if(isset($input['request']->data['idProduct']) &&  !empty($_SESSION['orderProducts'])){
			foreach($_SESSION['orderProducts'] as $key=>$product){
				if($input['request']->data['idProduct']==$product['Product']['id']){
					unset($_SESSION['orderProducts'][$key]);
				}
			}
		}
		...
		if($isRequestPost){
			...
			foreach($listOrderProduct as $key=>$data)
			{
				...
				if(isset($data['Product']['codeDiscount']) &&  
					...
					in_array($dataSend['codeDiscount'], $data['Product']['codeDiscount']) && 
			...
			$_SESSION['orderProducts']= $listOrderProduct;
			if(isset($dataSend['codeDiscount']) && $dataSend['codeDiscount']!='')
			{
