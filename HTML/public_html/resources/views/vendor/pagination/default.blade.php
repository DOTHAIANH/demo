...
@if ($paginator->lastPage() > 1)
    @if ($paginator->currentPage() > 1)
        <li class="{{ ($paginator->currentPage() == 1) ? ' disabled' : '' }} list-inline-item">
            <a href="{{ $paginator->url( $paginator->currentPage() - 1 ) }}" rel="First" class=""><i class="fa fa-angle-left"></i></a>
        </li>
    @endif
    ...
    @if ($paginator->currentPage() < $paginator->lastPage())
        <li class="{{ ($paginator->currentPage() == $paginator->lastPage()) ? ' disabled' : '' }} list-inline-item">
            <a href="{{ $paginator->url( $paginator->currentPage() + 1 ) }}" rel="next" class=""><i class="fa fa-angle-right"></i></a>
        </li>
    @endif
